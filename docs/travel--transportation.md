...
- [VMAzure/dealermax-mcp](https://github.com/VMAzure/dealermax-mcp): Public MCP server for the Italian automotive marketplace DealerMax — cross-dealer semantic search on used vehicles, long-term rental (NLT) offers, dealer directory, and automotive knowledge base. Registered on the Anthropic Registry as `app.dealermax/public-search`. Public, read-only, no API key.
