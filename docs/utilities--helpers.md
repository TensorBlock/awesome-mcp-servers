...
- [AI Applyd](https://github.com/aiapplyd/aiapplyd-mcp): Remote MCP server for job applications. ATS resume scoring with matched and missing keywords, per-role resume rewriting, cover letters, interview preparation, job matching, and auto-apply that submits on the employer's own hiring system across 12 ATS platforms. Transport: Streamable HTTP at `https://mcp.aiapplyd.com/mcp`. Auth: OAuth 2.1 with dynamic client registration. 10 tools, 3 prompts, 3 resources. Rated A on Glama. Install: nothing to install, add the URL.
