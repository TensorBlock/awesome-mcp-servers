...
- [Identik-me/identik-mcp](https://github.com/Identik-me/identik-mcp): Argentine electronic and digital signature (Ley 25.506) through Identik — list templates, create a document from a template, send it out for signature, track status, resend invitations, download the signed PDF with its audit certificate, fetch the audit trail, and cryptographically verify a signed PDF (integrity, certificate chain, RFC-3161 timestamp). Two remote Streamable HTTP endpoints: `https://electronica.identik.me/api/mcp` (electronic signature) and `https://firma.identik.me/api/mcp` (digital signature with licensed-CA certificates). Bearer API-token auth; published in the official MCP registry as `me.identik/electronica` and `me.identik/firma`. Sandbox environments available for evaluation.
- [alex-zwingli/plan-to-eat-mcp](https://github.com/alex-zwingli/plan-to-eat-mcp): Unofficial client for a personal [Plan to Eat](https://www.plantoeat.com) account — the meal-planning app. Recipe book CRUD (create from a URL or a payload, browse courses, cuisines and tags), the weekly planner (schedule recipes, prep notes and ingredient entries; move, duplicate, reorder within a slot, rescale servings, check for duplicates before scheduling), leftovers as a first-class flow, freezer portion tracking, and the shopping list Plan to Eat generates from the plan. 30 tools. A `plan-to-eat` CLI ships from the same tool registry, so the two surfaces cannot drift. Transport: stdio. Auth: `PLAN_TO_EAT_USERNAME` / `PLAN_TO_EAT_PASSWORD` env vars; the server logs in and caches the cookie session to disk. Clients: Claude Code (ships as a plugin with two skills), Claude Desktop, Cursor, and any stdio MCP host. License: MIT. Requires Node 18+. Install: `npx -y plan-to-eat-mcp` (or `npm i -g plan-to-eat-mcp`; the CLI is the `plan-to-eat` bin from the same package).
- [ToolsMonk](https://toolsmonk.com): Catalog lookup over 255 free browser-based tools in 10 categories (PDF, image, text, SEO, developer, calculators and everyday utilities), so an agent can find the right tool for a described task and hand back its URL. Read-only: it returns tool pages and does not upload, process or convert files. Five tools (`search_tools`, `get_tool`, `list_tools`, `list_categories`, `get_site_info`) plus 3 resources; ranked search accepts plain-language tasks such as "compress a PDF under 2MB". Streamable HTTP endpoint: `https://toolsmonk.com/api/mcp`. No auth, no API key, free, 120 requests/min per IP. Docs: https://toolsmonk.com/developers/mcp. Registry id `com.toolsmonk/catalog`.
