...
- [ThomasCrouzet/mcp-grand-lyon](https://github.com/ThomasCrouzet/mcp-grand-lyon): Local read-only MCP aggregating Métropole de Lyon open data (TCL, Vélo'v, parking, traffic, facilities, waste) behind ten business tools. Offline fixtures without credentials; live mode with DataGrandLyon account. Install: `uv sync` then `uv run grand-lyon-mcp serve --transport stdio`. Independent project, not affiliated with the Métropole. License: MIT.
