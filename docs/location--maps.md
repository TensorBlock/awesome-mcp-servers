...
- [kylehawke-stack/locationlists-mcp](https://github.com/kylehawke-stack/locationlists-mcp): Search, sample, count and buy US business location datasets (dealer networks, retail chains, contractors, healthcare providers). 9 tools, including filtered row queries and whole-CSV purchase by Stripe checkout or x402 (USDC on Base). Remote Streamable HTTP endpoint at `https://locationlists.com/mcp`, no auth.
