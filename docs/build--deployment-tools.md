...
- [friendlygeorge/docker-mcp-server](https://github.com/friendlygeorge/docker-mcp-server): Docker lifecycle management MCP server with 50 tools for container, image, volume, network, compose, and system operations. Agent-native structured JSON output. Install: `npx @supernova123/docker-mcp-server`. Stdio transport, no auth. MIT.
