...
- [getpapi/papi](https://github.com/getpapi/papi): Official PAPI MCP server. Persistent project management inside your AI coding tool: plan/build/review cycles, a live task board, decisions recorded with confidence levels, and build handoffs that survive across sessions and models. Hosted MCP endpoint at `https://mcp.getpapi.ai/mcp` (streamable HTTP, OAuth 2.1 with dynamic client registration) or run locally via `npx @papi-ai/server`. Listed on the official MCP Registry as `io.github.getpapi/papi`. Free in alpha.
