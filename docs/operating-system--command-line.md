...
- [Mac Developer Bridge](https://github.com/alexanderradahl/mac-developer-bridge): Local-first macOS MCP bridge that gives ChatGPT shell and file access, real PTY sessions, background jobs, and read-only Codex history. Intentionally unsandboxed with an explicit full-access acknowledgement. Source checkout; native stdio plus a Streamable HTTP/OAuth front end. MIT.
