...
- [Nexus Shell Agent Bridge](https://github.com/viewer12/Nexus-Shell-Releases): Allows approved local MCP clients to use existing Nexus Shell connections through visible SSH terminal tabs, SFTP transfers, SSH key management, and read-only monitoring metadata without exposing stored passwords or private keys. It provides 26 tools with first-use approval, revocation, and a local redacted audit log.
