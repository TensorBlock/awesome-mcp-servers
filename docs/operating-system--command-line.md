...
- [CMDOP Machine Care (`commandoperator/cmdop-care-mcp`)](https://github.com/commandoperator/cmdop-care-mcp): Read-only health and storage diagnostics for an already-enrolled CMDOP machine fleet. It exposes fleet roster, machine health projection, one bounded live diagnostic, and cmdop-owned storage inventory, while deliberately omitting shell execution, arbitrary file reads, directory listing, and open-ended machine delegation.
- [ostapondo/plonk](https://github.com/ostapondo/plonk): Drives a Mac's windows from an agent — snap zones, saved workspaces that reopen apps on the right monitor, keep-awake, screenshots and on-device OCR. Nineteen tools over stdio or Streamable HTTP, bridging to a loopback API on the user's own machine.
- [Mac Developer Bridge](https://github.com/alexanderradahl/mac-developer-bridge): Mac Developer Bridge gives ChatGPT access to the user's actual macOS execution environment over MCP: arbitrary zsh commands, unrestricted filesystem operations, persistent real PTY sessions, detached background jobs, and read-only discovery/reading of stored Codex threads. The bridge itself makes no model calls. It is intentionally not sandboxed and...
