...
- [minipainter](https://github.com/ArturSkowronski/minipainter): Search a bundled catalog of 1,607 miniature paints across Citadel, Army Painter, Vallejo and AK Interactive; find the nearest paint to a hex color or a text description; and read/update which paints the user owns. Cross-brand matching is grounded in the user's actual inventory, so an agent recommends paints you own before suggesting new ones. Seven...
