...
- [Gearotons/servomotor-mcp](https://github.com/Gearotons/servomotor-mcp): Drive Gearotons M17 open-source closed-loop NEMA-17 servomotors over an RS-485 bus from MCP clients. Discovers serial ports and motors, exposes firmware commands plus high-level move, sequence, status, and stop tools, and includes a mock backend for use without hardware. Install: `uvx --from servomotor-mcp servomotor-mcp` for mock mode, or `uvx --from "servomotor-mcp[serial]" servomotor-mcp` for real motors. MIT.
