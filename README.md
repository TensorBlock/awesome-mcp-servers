...
- [unitedideas/nothumansearch-mcp](https://github.com/unitedideas/nothumansearch-mcp) — Search engine for the agentic web. 960+ sites indexed and scored 0-100 on 7 agentic signals (llms.txt, ai-plugin.json, OpenAPI, MCP, API, robots.txt AI rules, Schema.org). Remote at `https://nothumansearch.ai/mcp`. Tools: `search_agents`, `get_site_details`, `get_stats`, `submit_site`, `register_monitor`, `verify_mcp`.
- [unitedideas/aidevboard-mcp](https://github.com/unitedideas/aidevboard-mcp) — Aggregated AI/ML/LLM engineering job listings across 55+ ATS sources. Remote at `https://aidevboard.com/mcp`. Tools: `search_jobs`, `get_job`, `list_companies`, `match_jobs`, `get_stats`.
