...
- [ejentum/ejentum-mcp](https://github.com/ejentum/ejentum-mcp) - Reasoning Harness for agentic AI: 4 cognitive operating modes (reasoning, code, anti-deception, memory) spanning 679 engineered abilities, served as MCP tools. Each call returns a structured scaffold (failure pattern, procedure, suppression vectors) the LLM absorbs before its first token. Catches sycophancy, hallucination, causal shortcuts, and reasoning decay at the moment they would otherwise emerge. MIT licensed, free tier 100 calls.
