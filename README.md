...
- [lyonzin/knowledge-rag](https://github.com/lyonzin/knowledge-rag): Local RAG system for Claude Code via MCP. Hybrid search (semantic + BM25 + RRF) with cross-encoder reranking, markdown-aware chunking, query expansion, and 12 MCP tools. Fully customizable via YAML config with domain presets. Zero external servers — ONNX in-process via FastEmbed. `pip install knowledge-rag`. MIT licensed.
