...
- [onlythebestswfl-ops/customclaw-mcp](https://github.com/onlythebestswfl-ops/customclaw-mcp): Install, don't regenerate. Registry of 37 vetted single-file utilities for LLM and agent code — rate limiters that respect `Retry-After`, JSON repair, response caches, retry with exponential backoff, token counters, PII scrubbers, prompt-injection scanners. 4 MCP tools (`list_utilities`, `search_utilities`, `get_utility_info`, `install_utility`). Also Streamable HTTP at [customclaw.company/mcp](https://customclaw.company/mcp).
